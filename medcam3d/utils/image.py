...
def scale_cam_image(cam: np.array, target_size=None) -> np.array: 
    """
    Scale cam image intensity to range of (0,1);
    Scale cam image size to target_size
    
    cam: shape: n, 1, z, y, x
    """
    result = []
    for img in cam:
        img = img - np.min(img)  # resize to range of (0,1)
        img = img / (1e-7 + np.max(img))
        if target_size is not None: 
            if len(target_size) == 3: # 3d image
                resize_fun = monai.transforms.Resize(target_size, mode='linear')
                img = resize_fun(img)
            else:
...
def cam_resize(cam: np.array, target_size=None) -> np.array: 
    """
    Scale cam image size to target_size
